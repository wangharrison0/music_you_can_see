...
function preload() {
    song = loadSound('assets/pianosong_of_storms.mp3');
}
...
/**fill(255, 10);
    ...
    for (i = 0; i < spectrumChunks.length; i++) {
        ...
        if (i % 3 == 0) { // turns circle and outline yellow
            fill(spectrumChunks[i], 0, 0.5*spectrumChunks[i], 10 * Math.log(spectrumChunks[i]));
            stroke(spectrumChunks[i], 0, 0.5 * spectrumChunks[i], 10 * Math.log(spectrumChunks[i]));
        }
        else if (i % 3 == 1) { // turns circle and outline yellow
            fill(0, 0, 3 * spectrumChunks[i], 10 * Math.log(spectrumChunks[i]));
            stroke(0, 0, 3 * spectrumChunks[i],  10 * Math.log(spectrumChunks[i]));
        }
        else if (i % 3 == 2) { // turns circle and outline yellow
            fill(0.4*spectrumChunks[i], 0, spectrumChunks[i], 10 * Math.log(spectrumChunks[i]));
            stroke(0.4*spectrumChunks[i], 0, spectrumChunks[i], 0, 10 * Math.log(spectrumChunks[i]));
        }
        ...
        for (r = -ampsArray[q]; r < ampsArray[q] + 1; r++) {
            {
                ...
                if (colors[q] % 3 == 0) { // draws yellow circles with indicated color. opacity scaled by colorscale
                    fill(colors[q], 0, 0.5*colors[q], colorscale[q]*10 * Math.log(colors[q]));
                    stroke(colors[q], 0, 0.5 * colors[q], colorscale[q] * 10 * Math.log(colors[q]));
                }
                else if (colors[q] % 3 == 1) { // red circles
                    fill(0, 0, 3*colors[q], colorscale[q] * 10 * Math.log(colors[q]));
                    stroke(0, 0, 3*colors[q], colorscale[q] * 10 * Math.log(colors[q]));
                }
                else if (colors[q] % 3 == 2) { // orange circles
                    fill(0.4*colors[q], 0, colors[q], colorscale[q] * 10 * Math.log(colors[q]));
                    stroke(0.4*colors[q], 0, colors[q], colorscale[q] * 10 * Math.log(colors[q]));
                }
